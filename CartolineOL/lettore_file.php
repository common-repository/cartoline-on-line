...
class ListaFile{
	...
	function get_directory_files(){
		$file_array=array();
		$file_count=1;
		$percorso=ABSPATH.'wp-content/uploads/cartoline/';
		if(is_dir($percorso)==false){
			mkdir($percorso);
		}
				...
				if ( !($file == "." || $file == ".." || is_dir($this->path.$file)) ) { 
					foreach ($scan_ext as $value) {
						if ($this->CheckExt($file, $value)) {
							...
							$file_array[$file_count]["time"]=filemtime($percorso.$file); 
							$file_array[$file_count]["size"]=filesize($percorso.$file);
							$file_array[$file_count]["write"]=is_writable($percorso.$file);
							$file_count++;
							break; 
						}
					}
